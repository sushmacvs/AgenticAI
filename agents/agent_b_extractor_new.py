...
class PlaywrightGeneratorAgent:
    ...
    def _generate_single_test(self, req: Requirement, state: AgentState) -> GeneratedTest:
        ...
        try:
            ...
            err = str(e)
            if "403" in err and ("zscloud" in err.lower() or "zscaler" in err.lower()):
                raise RuntimeError(
                    "Zscaler is blocking the Google GenAI API.\n"
                    "Open a browser, navigate to https://generativelanguage.googleapis.com/ "
                    "to authenticate with Zscaler, then re-run."
                ) from e
            raise
